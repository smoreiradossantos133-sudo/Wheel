...
pub mod llvm {
    ...
    pub fn compile_with_llvm(prog: &Program, out_path: &Path) -> Result<()> {
        ...
        for item in &prog.items {
            codegen_stmt(item, &context, &module, &builder, &printf, &locals, &main_fn, i64_t, i32_t);
        }
    ...
    fn codegen_stmt<'ctx>(
        ...
        match stmt {
            Stmt::Let { name, value } => {
                let val = match value {
                    Expr::Int(_) | Expr::BinaryOp { .. } | Expr::Ident(_) => {
                        gen_expr(value, context, module, builder, locals, i64_t).into_int_value()
                    }
                    Expr::Str(s) => {
            ...
            Stmt::Assign { name, value } => {
                let val = gen_expr(value, context, module, builder, locals, i64_t).into_int_value();
                if let Some(ptr) = locals.get(name) {
                    builder.build_store(*ptr, val);
                }
            }
            Stmt::Expr(Expr::Call { name, args }) if name == "print" && args.len() == 1 => {
                ...
                match &args[0] {
                    Expr::Int(v) => {
                        ...
                        let val = gen_expr(&args[0], context, module, builder, locals, i64_t);
                    ...
                    Expr::Ident(id) => {
                        ...
                        if let Some(ptr) = locals.get(id) {
                            ...
                            let v = builder.build_load(elem_type, *ptr, &format!("load_{}", id));
                            let fmt_ptr = unsafe { module.get_global("_fmt_ld").unwrap().as_pointer_value() };
            ...
            Stmt::If { cond, then_body, else_body } => {
                // evaluate condition
                let cond_val = gen_expr(cond, context, module, builder, locals, i64_t).into_int_value();
                ...
                for s in then_body {
                    codegen_stmt(s, context, module, builder, printf, locals, main_fn, i64_t, i32_t);
                }
    ...
    fn gen_expr<'ctx>(
        ...
        match e {
            ...
            Expr::Ident(name) => {
                if let Some(ptr) = locals.get(name) {
                    let elem_type = ptr.get_type().get_element_type();
                    builder.build_load(elem_type, *ptr, &format!("load_{}", name))
                } else {
                    i64_t.const_int(0, false).into()
                }
            }
            Expr::BinaryOp { op, left, right } => {
                let l = gen_expr(left, context, module, builder, locals, i64_t).into_int_value();
                let r = gen_expr(right, context, module, builder, locals, i64_t).into_int_value();
